      ...
      <div className="space-y-6 max-w-xl">
        <div className="bg-[#2d2d2d] border border-white/10 rounded-[20px] p-6">
          ...
          <form onSubmit={handleNameSave} className="space-y-4">
            {nameMsg.text && (
              <div className={`p-3 rounded-[4px] text-sm ${nameMsg.type === 'success' ? 'bg-[#3cffd0]/10 text-[#3cffd0]' : 'bg-red-500/10 text-red-400'}`}>
                {nameMsg.text}
              </div>
            )}
            <div>
        ...
        <div className="bg-[#2d2d2d] border border-white/10 rounded-[20px] p-6">
          ...
          <form onSubmit={handleFaviconSave} className="space-y-4">
            {faviconMsg.text && (
              <div className={`p-3 rounded-[4px] text-sm ${faviconMsg.type === 'success' ? 'bg-[#3cffd0]/10 text-[#3cffd0]' : 'bg-red-500/10 text-red-400'}`}>
                {faviconMsg.text}
              </div>
            )}
            {faviconUrl && (
              <div className="flex items-center gap-3 mb-2">
                <img src={faviconUrl} alt="Favicon 预览" className="w-10 h-10 rounded border border-white/10 object-contain" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
