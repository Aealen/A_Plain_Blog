    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-card border border-border rounded-[20px] p-6">
          ...
          <p className="text-3xl font-bold font-display mt-2 text-primary">{totalPv.toLocaleString()}</p>
        </div>
        <div className="bg-card border border-border rounded-[20px] p-6">
          <p className="font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground">30 天 UV</p>
          <p className="text-3xl font-bold font-display mt-2 text-primary">{totalUv.toLocaleString()}</p>
        </div>
        <div className="bg-card border border-border rounded-[20px] p-6">
              ...
              <tbody className="divide-y divide-border">
                {dailyStats.length === 0 ? (
                  ...
                  dailyStats.sort((a, b) => b.date.localeCompare(a.date)).map((stat) => (
                    <tr key={stat.date} className="hover:bg-hover transition-colors">
                      <td className="px-4 py-3 text-sm text-foreground font-mono">{stat.date}</td>
                      <td className="px-4 py-3 text-sm text-primary text-right font-mono">{stat.pv.toLocaleString()}</td>
              ...
              topPages.map((page, index) => (
                <div key={page.path} className="p-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground font-mono shrink-0">
                    <span>PV: <span className="text-primary">{page.pv.toLocaleString()}</span></span>
