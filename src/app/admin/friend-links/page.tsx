...
export default function FriendLinksPage() {
  ...
  const inputClass = "w-full px-3 py-2 border border-border rounded-[4px] bg-card text-foreground focus:outline-none focus:border-primary"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div className="mb-2 flex gap-2">
                ...
                <button
                  ...
                  className="shrink-0 px-3 py-2 border border-border rounded-[4px] bg-card/50 text-foreground hover:bg-hover transition-colors text-sm"
                >
                  上传
                </button>
              </div>
              {showAvatarUploader && (
                <div className="mb-2 border border-border rounded-[4px] p-3 bg-card">
        ...
        <div className="lg:col-span-2 bg-card rounded-[24px] border border-border">
          ...
          <div className="divide-y divide-border">
            {links.map(link => (
              <div key={link.id} className="p-4 flex items-center justify-between hover:bg-hover">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-mono uppercase ${
                        link.isActive ? 'bg-primary/20 text-primary' : 'bg-card/50 text-muted-foreground'
                      }`}>
                        {link.isActive ? '启用' : '禁用'}
                      </span>
                    </div>
                    <a href={link.url} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:text-link-hover transition-colors">{link.url}</a>
                    {link.description && <p className="text-sm text-muted-foreground mt-0.5">{link.description}</p>}
                  </div>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => handleToggle(link.id)} className={`text-sm font-medium transition-colors ${link.isActive ? 'text-yellow-400 hover:text-yellow-300' : 'text-primary hover:text-link-hover'}`}>
                    {link.isActive ? '禁用' : '启用'}
                  </button>
                  <button onClick={() => handleEdit(link)} className="text-sm text-primary hover:text-link-hover font-medium transition-colors">编辑</button>
