...
const inputClass = "w-full px-3 py-2 border border-border rounded-[4px] bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-colors"
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold font-display uppercase tracking-[1px] text-foreground">编辑文章</h1>
        <Link href="/admin/articles" className="text-sm font-mono text-muted-foreground hover:text-foreground transition-colors">← 返回列表</Link>
      </div>

      {error && <div className="bg-destructive/10 text-destructive border border-destructive/30 p-3 rounded-[4px] mb-4 text-sm">{error}</div>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-card rounded-[24px] border border-border p-6 space-y-4">
            <div>
              <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">标题</label>
            ...
            <div>
              <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">内容</label>
              <MarkdownEditor value={formData.content} onChange={value => setFormData({ ...formData, content: value })} />
            </div>
            <div>
              <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">摘要</label>
          ...
          <div className="bg-card rounded-[24px] border border-border">
            ...
            {showSEO && (
              <div className="px-6 pb-6 space-y-4 border-t border-border">
                <div className="pt-4">
                  <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">SEO 标题</label>
                ...
                <div>
                  <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">SEO 描述</label>
                ...
                <div>
                  <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">SEO 关键词</label>
        ...
        <div className="space-y-6">
          <div className="bg-card rounded-[24px] border border-border p-6 space-y-4">
            <button onClick={() => handleUpdate('PUBLISHED')} disabled={saving || !formData.title} className="w-full bg-primary text-primary-foreground py-2 rounded-[24px] hover:bg-primary/90 font-mono uppercase text-sm tracking-[1px] transition-colors disabled:opacity-50">更新并发布</button>
            <button onClick={handleSaveDraft} disabled={saving || !formData.title} className="w-full bg-card/50 text-foreground py-2 rounded-[24px] hover:bg-hover font-mono uppercase text-sm tracking-[1px] transition-colors disabled:opacity-50">保存修改</button>
            {hasDraft && (
              <button onClick={handleDiscardDraft} disabled={saving} className="w-full text-destructive border border-destructive/30 py-2 rounded-[24px] hover:bg-destructive/10 transition-colors disabled:opacity-50 text-sm font-mono uppercase tracking-[1px]">撤销改动</button>
            )}
            <Link href="/admin/articles" className="block w-full text-center py-2 text-sm font-mono text-muted-foreground hover:text-foreground transition-colors">取消</Link>
          </div>

          <div className="bg-card rounded-[24px] border border-border p-6">
            <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-2">状态</label>
          ...
          <div className="bg-card rounded-[24px] border border-border p-6">
            <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-2">分类</label>
            <div className="flex flex-wrap gap-2">
              {categories.map(cat => (
                <button key={cat.id} type="button" onClick={() => toggleCategory(cat.id)} className={`text-sm px-3 py-1 rounded-full border transition-colors ${
                  formData.categoryIds.includes(cat.id) ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-foreground border-border hover:border-primary'
                }`}>{cat.name}</button>
              ))}
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button
                ...
                className="shrink-0 px-3 py-2 border border-border rounded-[4px] bg-accent text-foreground hover:bg-accent/80 transition-colors text-sm font-mono uppercase tracking-[1px] disabled:opacity-50"
              >
                {creatingCategory ? '...' : '新建'}
              </button>
            </div>
            {newCategoryError && <p className="mt-1 text-xs text-destructive">{newCategoryError}</p>}
          </div>

          <div className="bg-card rounded-[24px] border border-border p-6">
            <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-2">标签</label>
            <div className="flex flex-wrap gap-2">
              {tags.map(tag => (
                <button key={tag.id} type="button" onClick={() => toggleTag(tag.id)} className={`text-sm px-3 py-1 rounded-full border transition-colors ${
                  formData.tagIds.includes(tag.id) ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-foreground border-border hover:border-primary'
                }`}>{tag.name}</button>
              ))}
            ...
            {newTagError && <p className="mt-1 text-xs text-destructive">{newTagError}</p>}
          </div>

          <div className="bg-card rounded-[24px] border border-border p-6 space-y-4">
            <div>
              <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">封面图片</label>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="shrink-0 px-3 py-2 border border-border rounded-[4px] bg-accent text-foreground hover:bg-accent/80 transition-colors text-sm font-mono uppercase tracking-[1px]"
                >
                  上传图片
                </button>
              </div>
              {formData.coverImage && (
                <div className="mt-2">
                  <img src={formData.coverImage} alt="封面预览" className="w-full h-32 object-cover rounded-[4px] border border-border" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
                </div>
              )}
            </div>

            {showCoverUploader && (
              <div className="border border-border rounded-[4px] p-4 bg-card/50">
            ...
            <div>
              <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">
                URL Slug
              ...
              <input
                ...
                className={`${inputClass} ${slugError ? 'border-destructive focus:ring-destructive/50' : ''}`}
              />
            ...
            <div>
              <label className="block font-mono text-[12px] uppercase tracking-[1.5px] text-muted-foreground mb-1">创建时间</label>
              <input
                type="datetime-local"
